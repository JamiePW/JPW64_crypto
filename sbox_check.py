...
def fixed_point(sbox, n=16, trans=False):
    """
    @SBox:   S盒列表
    @trans:  所给S盒内元素是否经过转化，十进制则没经过转化，字母形式则经过转化
    @return: 存在不动点、反不动点返回 False，否则返回 True
    """
    for r in range(n):
        for c in range(n):
            tmp = transform(sbox[r][c]) if not trans else sbox[r][c]
            ### 1.判断是否是不动点
            if translis[r] + translis[c] == tmp:
                return False
            ### 2.判断是否是反不动点
            if translis[15 - r] + translis[15 - c] == tmp:
                return False
    return True
...
# 储存每一个不可约多项式对应的 c的个数、总和等，最后输出到csv文件中
data = []
# 所有不可约多项式对应的十进制整数
ms = [
    283, 285, 299, 301, 313, 319, 333, 351, 355, 357, 361, 369, 375, 379, 391,
    395, 397, 415, 419, 425, 433, 445, 451, 463, 471, 477, 487, 499, 501, 505
]
...
for m in ms:
    ...
    ans.insert(0, m)
    data.append(ans)
    print(f"m={m} is finish.")

#############################################################################################################3
# 5.存入csv文件
import csv

with open("所有m-S盒对应c.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["m(x)", "number of C", "C"])
