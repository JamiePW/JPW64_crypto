import csv

Sbox = [
    
]

def hw(target):
    # 计算汉明重量
    return bin(target).count("1")

LAT = [[-128 for i in range(1 << 8)] for j in range(1 << 8)]    #初始化时即减去2^(m-1)

for a in range(256):
    for b in range(256):
        for x in range(256):
            tmp1 = hw(x & a)
            tmp2 = hw(Sbox[x] & b)
            if (tmp1 % 2) == (tmp2 % 2):    # 本质上是统计与掩码逐比特与后串中1的个数奇偶性是否相同
                LAT[a][b] += 1


# print(LAT)

with open ("jpw64-LAT.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerows(LAT)

max = 0
for i in range(256):
    for j in range(256):
        if abs(LAT[i][j]) > max and abs(LAT[i][j]) != 128:
            max = abs(LAT[i][j])

print("max absolute value of LAT: " + str(max))